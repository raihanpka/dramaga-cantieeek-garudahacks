...
var chunkTWP22Z2C_cjs = require('./chunk-TWP22Z2C.cjs');
...
var ExecutionEngine = class extends chunk73LEILKH_cjs.MastraBase {
  mastra;
  constructor({ mastra }) {
  ...
  async executeSleep({
    ...
    if (fn) {
      ...
      duration = await fn({
        ...
        writer: new chunkTWP22Z2C_cjs.ToolStream(
          {
            prefix: "step",
            callId: stepCallId,
            name: "sleep",
...
function createStep(params) {
  const wrapExecute = (execute) => {
    return async (executeParams) => {
      ...
      if (params instanceof chunkZHBAIO3U_cjs.Agent || params instanceof chunk4Z3OU5RY_cjs.Tool) {
  ...
  if (params instanceof chunkZHBAIO3U_cjs.Agent) {
    return {
      id: params.name,
      // @ts-ignore
      ...
      execute: wrapExecute(async ({ inputData, [chunkBB6DPGIV_cjs.EMITTER_SYMBOL]: emitter, runtimeContext, abortSignal, abort }) => {
        let streamPromise = {};
...
var Workflow = class extends chunk73LEILKH_cjs.MastraBase {
  id;
  description;
  inputSchema;
  outputSchema;
