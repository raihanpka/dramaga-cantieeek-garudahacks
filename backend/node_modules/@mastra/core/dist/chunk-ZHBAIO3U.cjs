...
var chunkTCS63CEG_cjs = require('./chunk-TCS63CEG.cjs');
var chunkF4BRBWD2_cjs = require('./chunk-F4BRBWD2.cjs');
var chunk2CF2A2HU_cjs = require('./chunk-2CF2A2HU.cjs');
var chunkO3YMKNEP_cjs = require('./chunk-O3YMKNEP.cjs');
var chunkA5BO52EQ_cjs = require('./chunk-A5BO52EQ.cjs');
var chunkRTZD7HJT_cjs = require('./chunk-RTZD7HJT.cjs');
...
function runScorer({
  ...
  chunkA5BO52EQ_cjs.executeHook("onScorerRun" /* ON_SCORER_RUN */, payload);
}
...
var MastraAgentStream = class extends web.ReadableStream {
  #usageCount = {
    promptTokens: 0,
    completionTokens: 0,
    totalTokens: 0
  ...
  get textStream() {
    return this.pipeThrough(new web.TransformStream({
...
_Agent_decorators = [chunkO3YMKNEP_cjs.InstrumentClass({
  prefix: "agent",
...
exports.Agent = class Agent extends (_a = chunk73LEILKH_cjs.MastraBase) {
  id;
  name;
  #instructions;
  #description;
  ...
  constructor(config) {
    ...
    if (config.voice) {
      ...
      this.#voice = new chunkTCS63CEG_cjs.DefaultVoice();
    }
  ...
  async getVoice({
    ...
    if (this.#voice) {
      ...
      return new chunkTCS63CEG_cjs.DefaultVoice();
    }
  }
  get instructions() {
  ...
  get tools() {
    ...
    return chunkRTZD7HJT_cjs.ensureToolProperties(this.#tools);
  }
  getTools({
    runtimeContext = new chunkLABUWBKX_cjs.RuntimeContext()
  } = {}) {
    ...
    return resolveMaybePromise(result, tools => {
      ...
      return chunkRTZD7HJT_cjs.ensureToolProperties(tools);
    });
  }
  get llm() {
  ...
  getLLM({
    ...
    return resolveMaybePromise(modelToUse, resolvedModel => {
      const llm = new chunk2CF2A2HU_cjs.MastraLLM({
        model: resolvedModel,
        mastra: this.#mastra
      });
      if (this.#primitives) {
  ...
  async generateTitleFromUserMessage({
    ...
    const normMessage = new chunkF4BRBWD2_cjs.MessageList().add(message, "user").get.all.ui().at(-1);
    if (!normMessage) {
  ...
  async genTitle(userMessage, runtimeContext, model, instructions) {
    try {
      if (userMessage) {
        const normMessage = new chunkF4BRBWD2_cjs.MessageList().add(userMessage, "user").get.all.ui().at(-1);
        if (normMessage) {
  ...
  async fetchMemory({
    ...
    userMessages,
    systemMessage,
    messageList = new chunkF4BRBWD2_cjs.MessageList({
      threadId,
      resourceId
    }),
    runtimeContext = new chunkLABUWBKX_cjs.RuntimeContext()
...
`) ?? void 0;
      ...
      const returnList = new chunkF4BRBWD2_cjs.MessageList().addSystem(systemMessages).add(processedMemoryMessages, "memory").add(newMessages, "user");
      return {
        threadId: thread.id,
  ...
  async getAssignedTools({
    ...
    const assignedCoreToolEntries = await Promise.all(assignedToolEntries.map(async ([k, tool]) => {
      ...
      return [k, chunkRTZD7HJT_cjs.makeCoreTool(tool, options)];
    }));
    ...
    if (toolsFromToolsets.length > 0) {
      ...
      for (const toolset of toolsFromToolsets) {
        for (const [toolName, tool] of Object.entries(toolset)) {
          ...
          const convertedToCoreTool = chunkRTZD7HJT_cjs.makeCoreTool(toolObj, options, "toolset");
  ...
  async getClientTools({
    ...
    if (clientToolsForInput.length > 0) {
      ...
      for (const [toolName, tool] of clientToolsForInput) {
        ...
        const convertedToCoreTool = chunkRTZD7HJT_cjs.makeCoreTool(rest, options, "client-tool");
        toolsForRequest[toolName] = convertedToCoreTool;
      }
    }
    return toolsForRequest;
  ...
  async convertTools({
    ...
    if (this.#mastra) {
      mastraProxy = chunkRTZD7HJT_cjs.createMastraProxy({
        mastra: this.#mastra,
        logger
      });
    }
  ...
  __primitive({
    ...
    return {
      before: async () => {
        ...
        const messageList = new chunkF4BRBWD2_cjs.MessageList({
          threadId,
          resourceId,
          generateMessageId,
        ...
        let [memoryMessages, memorySystemMessage] = thread.id && memory ? await Promise.all([this.getMemoryMessages({
          ...
          vectorMessageSearch: new chunkF4BRBWD2_cjs.MessageList().add(messages, `user`).getLatestUserContent() || "",
          memoryConfig,
          runtimeContext
...
${(() => {
            let result = ``;
            const messages2 = new chunkF4BRBWD2_cjs.MessageList().add(resultsFromOtherThreads, "memory").get.all.v1();
            let lastYmd = null;
...
`) ?? void 0;
        ...
        const processedList = new chunkF4BRBWD2_cjs.MessageList({
          threadId: threadObject.id,
          resourceId,
          // @ts-ignore Flag for agent network messages
          _agentNetworkAppend: this._agentNetworkAppend
      ...
      after: async ({
        ...
        const messageListResponses = new chunkF4BRBWD2_cjs.MessageList({
          threadId,
          resourceId,
          // @ts-ignore Flag for agent network messages
          _agentNetworkAppend: this._agentNetworkAppend
  ...
  async #runScorers({
    ...
    if (Object.keys(this.evals || {}).length > 0) {
      for (const metric of Object.values(this.evals || {})) {
        chunkA5BO52EQ_cjs.executeHook("onGeneration" /* ON_GENERATION */, {
          input,
          output: outputText,
          runId: runIdToUse,
          metric,
  ...
  streamVNext(messages, streamOptions) {
    ...
    return new MastraAgentStream({
      ...
      createStream: async (writer, onResult) => {
        const defaultStreamOptions = await defaultStreamOptionsPromise;
        const mergedStreamOptions = {
          ...defaultStreamOptions,
          ...streamOptions,
...
var LegacyWorkflow = class extends chunk73LEILKH_cjs.MastraBase {
  name;
  triggerSchema;
  resultSchema;
  resultMapping;
