...
_Mastra_decorators = [chunkO3YMKNEP_cjs.InstrumentClass({
  prefix: "mastra",
...
exports.Mastra = class Mastra {
  ...
  constructor(config) {
    ...
    this.#telemetry = chunkO3YMKNEP_cjs.Telemetry.init(config?.telemetry);
...
`
    ...
    chunkA5BO52EQ_cjs.registerHook("onScorerRun" /* ON_SCORER_RUN */, createOnScorerHook(this));
    this.setLogger({
      logger
    });
  }
  ...
  setTelemetry(telemetry) {
    this.#telemetry = chunkO3YMKNEP_cjs.Telemetry.init(telemetry);
    if (this.#agents) {
      Object.keys(this.#agents).forEach(key => {
        if (this.#telemetry) {
